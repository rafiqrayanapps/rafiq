...
export default function SplashPage() {
  ...
  const appName = (generalConfig?.appName || aboutConfig?.appName || aboutConfig?.title || "رفيق المصمم").trim();
  const appLogoUrl = (generalConfig?.appLogo || aboutConfig?.appLogoImage || aboutConfig?.logoImage || "").trim();
  
  // Description: only show if explicitly set and not empty in admin control panel
  const appSubtitle = (
    generalConfig?.appSubtitle !== undefined 
      ? (generalConfig.appSubtitle || '') 
      : (aboutConfig?.subtitle || '')
  ).trim();

  // Welcome message: only show if explicitly set and not empty in admin control panel
  const splashWelcomeText = (
    generalConfig?.splashWelcomeText !== undefined 
      ? (generalConfig.splashWelcomeText || '') 
      : ''
  ).trim();

  const words = (appName || "رفيق المصمم").split(/\s+/).filter(Boolean);
  const topWord = words.length > 1 ? words[0] : '';
  const bottomWord = words.length > 1 ? words.slice(1).join(' ') : (words[0] || 'رفيق');

  useEffect(() => {
    // Prefetch /home early so chunks and RSC payloads are ready
    try {
